...
int main(int argc, char *argv[])
{
  ...
  int motDeviner[15];
  int entre = 0, positionCaractere = 0, j = 0;
  ...
  while(essaiRestant > 0) /* Boucle principale */
      ...
      for (j = 0; j < longeurMot; j++)
	{
      ...
      positionCaractere = comparerCaractere(entre, mot, motDeviner, longeurMot);

      if(positionCaractere != entre)
	{
	  essaiRestant--;
	  printf("Il reste %d essai restant avant une mort certaine...\n",essaiRestant);
	}
